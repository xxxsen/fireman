...
- 市场数据任务化 worker + 全局市场资产目录：Go 建任务、sidecar 执行、Go post-process 落库；计划持仓直接引用 `market_assets.asset_key`，缺历史资产由模拟 readiness 检查与一键同步补齐；模拟历史快照带 `source_hash` 审计
...
├── web/                         # Next.js 前端
├── sidecars/market-provider/    # 市场数据 worker sidecar（AKShare/TickFlow）
