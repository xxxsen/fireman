  ...
  return (
    <section aria-label="FIRE 快算结论" className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <Metric label="计划 FIRE 时资产" value={formatMoney(result.projected_assets_at_fire_minor)} />
        <Metric label="所需 FIRE 资本" value={formatMoney(result.required_assets_at_fire_minor)} />
        <Metric
          label="资金富余 / 缺口"
          value={formatMoney(result.fire_funding_gap_minor)}
          tone={result.fire_funding_gap_minor >= 0 ? "positive" : "danger"}
        />
        <Metric label="最早可 FIRE" value={earliest} />
        <Metric label="可完整支付" value={supportLabel(result.support_months_after_fire)} />
        <Metric label="耗尽年龄" value={depletion} />
        <Metric label="期末名义资产" value={formatMoney(result.terminal_wealth_minor)} />
        <Metric label="期末真实资产" value={formatMoney(result.real_terminal_wealth_minor)} />
        <Metric label="实际年化收益" value={formatPercent(result.real_annual_return_rate)} />
      </div>
