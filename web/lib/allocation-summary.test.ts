...
describe("buildAllocationSummary", () => {
  it("builds asset class and active region rows", () => {
    ...
    expect(rows[0].current_weight).toBeCloseTo(0.6);
    expect(rows[0].gap_amount_minor).toBe(0);
    expect(rows[1].gap_amount_minor).toBe(10_000);
...
const rebalanceLines: RebalanceLine[] = targets.holdings.map((holding) => ({
  ...holding,
  action: holding.deviation_amount_minor >= 0 ? "increase" : "decrease",
  suggested_trade_minor: Math.abs(holding.deviation_amount_minor),
}));
