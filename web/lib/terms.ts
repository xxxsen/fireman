...
export const HELP_TOPICS = {
  ...
  investment_path_total_contribution: calculatedTopic(
    "投入路径累计投入",
    "主窗口内从账户外部实际投入的本金总额。",
    "起点额外本金加上窗口内每笔计划月投；买入费用不从累计投入中扣除。",
    "使用本次冻结的起点本金、月投金额、计划日和窗口长度。",
    "它是本金基准，不是买入后的资产市值，也不包含投资收益。",
  ),
  investment_path_terminal_value: calculatedTopic(
    "投入路径期末资产",
    "窗口最后一个估值日的资产市值与现金余额之和。",
    "期末持有单位按最后有效价格估值，再加账户现金。",
    "使用本次历史价格、实际成交、交易费用和现金账本。",
    "可与累计投入比较整体盈亏；它只是该段历史的终点，不是未来预测。",
  ),
  investment_path_profit: calculatedTopic(
    "投入路径投资损益",
    "期末资产相对累计外部投入的金额差。",
    "期末资产 − 累计投入。",
    "使用同一主窗口的期末资产和累计投入。",
    "正数表示该窗口最终盈利，负数表示最终仍低于投入本金；它不是收益率。",
  ),
  investment_path_xirr: calculatedTopic(
    "投入路径 XIRR",
    "考虑每笔外部投入实际发生日期的资金加权年化收益率。",
    "以每笔投入为负现金流、期末资产为正现金流，求使净现值为零的年化收益率。",
    "使用主窗口或滚动窗口的实际外部现金流日期、金额和期末资产。",
    "它更接近用户资金的实际年化体验；投入时点不同会改变结果。",
    { caveat: "现金流不变号或业务范围内无有效根时不可计算；它不等于资产本身的时间加权收益。" },
  ),
  investment_path_twr_annualized: calculatedTopic(
    "投入路径时间加权年化",
    "剔除新增本金规模影响后，账户每单位资金的复合年化收益。",
    "外部资金流入前按当时单位净值增发账户单位，先计算单位净值总收益，再按窗口实际天数年化。",
    "使用逐日账户价值、外部现金流和窗口天数。",
    "适合观察资产与策略本身的增长，不反映用户在不同时点投入了多少钱。",
  ),
  investment_path_max_drawdown: calculatedTopic(
    "投入路径单位净值最大回撤",
    "剔除新增本金影响后，单位净值从历史峰值跌到后续低点的最深跌幅。",
    "逐日计算单位净值 ÷ 此前峰值 − 1，并取最小值。",
    "使用窗口内现金流发生前口径的单位净值路径。",
    "结果以负数展示，越负表示经历过的峰谷损失越深；新增月投本身不会制造收益或回撤。",
  ),
  investment_path_below_principal: calculatedTopic(
    "投入路径低于本金时间",
    "账户价值连续低于截至当日累计投入本金的最长自然日数。",
    "逐日比较账户价值与累计投入，从首次低于本金起连续计日，回到本金及以上时结束。",
    "使用逐日账户价值和逐日累计外部投入。",
    "它描述用户等待回本的体验，不等于从历史峰值恢复所需的回撤持续期。",
  ),
  investment_path_transaction_cost: calculatedTopic(
    "投入路径交易成本",
    "主窗口内所有实际买入、卖出或再平衡产生的费用合计。",
    "每笔费用按成交预算 × 冻结交易成本率取整后累加。",
    "使用实际成交记录和本次冻结的统一交易成本率。",
    "费用已经从账户现金中扣除并反映在期末资产、XIRR 和时间加权收益里。",
  ),
  investment_path_account_path: calculatedTopic(
    "投入路径账户价值与本金路径",
    "把每个估值日的账户价值与截至当日累计投入放在同一金额轴上。",
    "账户价值为资产市值加现金；累计投入只累加外部现金流。",
    "使用主窗口逐日账本点。",
    "账户线高于投入线表示当日整体盈利，低于投入线表示当日仍有本金浮亏。",
  ),
  investment_path_rolling_windows: calculatedTopic(
    "投入路径滚动窗口",
    "用相同月投、成本和持有月数，从历史中不同月份重复开始实验。",
    "从评估范围内每个具备完整行情覆盖的计划日起，运行固定长度窗口。",
    "使用本次冻结历史、统一策略参数和所有合格月度起点。",
    "用于观察结果对入场月份的敏感性；相邻窗口高度重叠，不是独立样本或未来概率。",
  ),
  investment_path_quantiles: calculatedTopic(
    "投入路径期末资产分位数",
    "把所有合格滚动窗口的期末资产从低到高排列后的 P10、P50 和 P90。",
    "Pn 表示约有 n% 的历史窗口终值不高于该值，非整数位置使用线性插值。",
    "使用同一策略的全部合格滚动窗口期末资产。",
    "P10 是偏不利历史结果，P50 是中位数，P90 是偏有利历史结果；都不是未来收益概率。",
  ),
  investment_path_drawdown_quantiles: calculatedTopic(
    "投入路径最大回撤分位数",
    "把每个滚动窗口的单位净值最大回撤按数值从低到高排列后的 P10、P50 和 P90。",
    "先计算每个窗口的最大回撤，再用线性插值取得三个分位位置。",
    "使用同一策略全部合格滚动窗口的最大回撤。",
    "回撤以负数保存，因此 P10 通常更负、代表更深回撤，P90 更接近零；不是未来概率。",
  ),
  investment_path_best_worst_start: calculatedTopic(
    "投入路径最佳与最差起点",
    "在全部合格滚动窗口中，期末资产最低和最高的计划起始日期。",
    "按期末资产比较窗口；最低者为最差起点，最高者为最佳起点。",
    "使用同一策略、相同投入与固定持有期的滚动窗口。",
    "只按期末资产排名，不同时评价回撤、回本时间或未来适合入场的日期。",
  ),
