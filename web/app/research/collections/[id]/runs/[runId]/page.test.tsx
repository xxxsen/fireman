...
describe("ResearchRunDetailPage", () => {
  ...
  it("renders metric cards, charts, tables and export entries when succeeded", async () => {
    ...
    expect(screen.getAllByRole("button", { name: "查看「收益贡献」说明" }).length).toBeGreaterThan(0);
    expect(screen.getAllByRole("button", { name: "查看「风险贡献」说明" }).length).toBeGreaterThan(0);
    expect(screen.getAllByRole("button", { name: "查看「回撤贡献」说明" }).length).toBeGreaterThan(0);
