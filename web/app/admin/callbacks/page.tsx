...
import {
  CALLBACK_TABLE_HEADERS,
  CallbackTableRows,
} from "@/components/admin/CallbackTable";
import { Alert } from "@/components/ui/Alert";
import {
  CALLBACK_RESULT_LABELS,
  listAdminPostProcessRecords,
...
const RESULT_OPTIONS = [
  { value: "", label: "全部结果" },
  ...Object.entries(CALLBACK_RESULT_LABELS).map(([value, label]) => ({ value, label })),
];
...
function CallbacksBoard() {
  ...
  const query = useQuery({
    queryKey: ["admin", "callbacks", { result, taskType, taskId, offset: params.offset }],
    queryFn: () =>
      listAdminPostProcessRecords({
  ...
  return (
    <div data-testid="admin-callbacks">
      <p className="mb-4 text-sm text-ink-muted">
        每条记录对应 sidecar 的一次 post-process 回调；可重试失败会由 sidecar
        退避重试，永久失败任务需人工处理。
      </p>
      ...
      <AdminTable
        headers={CALLBACK_TABLE_HEADERS}
        isLoading={query.isLoading}
        error={
          query.isError && page === undefined
            ? queryErrorMessage(query.error, "回调记录加载失败")
            : null
        }
        ...
        empty={{
          ...
          description: params.dirty
            ? "尝试调整筛选条件。"
            : "sidecar 完成任务并回调后，记录会出现在这里。",
        }}
      >
        {page && <CallbackTableRows items={page.items} />}
      </AdminTable>
