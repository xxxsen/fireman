...
function TaskLink({ rule }: { rule: AdminAutoUpdateRule }) {
  ...
  return (
    <span className="inline-flex max-w-48 items-center gap-1 overflow-hidden">
      ...
      {rule.task?.status && <span className="shrink-0 text-ink-muted">({rule.task.status})</span>}
    </span>
  );
}
...
function ruleStatus(rule: AdminAutoUpdateRule): string {
  ...
  if (rule.task && ["pending", "running", "pre_complete"].includes(rule.task.status)) {
    return "任务执行中";
  }
  if (rule.task && ["failed", "canceled"].includes(rule.task.status)) {
    return "最近失败";
  }
  if (rule.last_failed_at && rule.last_failed_at > (rule.last_success_at ?? 0)) {
    return "最近失败";
  }
  ...
  return (
    <div className="flex min-w-36 items-center gap-1">
      ...
      {changed && <>
        ...
        <button type="button" disabled={pending} className="text-ink-muted disabled:opacity-60" onClick={() => setDraft(rule.interval_hours)}>取消</button>
      </>}
    </div>
  );
}
...
function RuleCells({
  ...
  return <>
    ...
    <td><IntervalEditor key={`${rule.id}-${rule.version}`} rule={rule} pending={pending} onSave={(hours) => onUpdate(rule.enabled, hours)} /></td>
    <td>{rule.next_run_at ? formatDateTimeFromMs(rule.next_run_at) : "--"}</td>
    <td>{rule.last_success_at ? formatDateTimeFromMs(rule.last_success_at) : "--"}</td>
    <td className="max-w-48 truncate" title={rule.last_error_message || undefined}>
      {rule.last_failed_at ? formatDateTimeFromMs(rule.last_failed_at) : "--"}
      {rule.last_error_code ? ` (${rule.last_error_code})` : ""}
    </td>
    <td><TaskLink rule={rule} /></td>
...
function AutoUpdatesContent() {
  ...
  const updateRule = async (rule: AdminAutoUpdateRule, active: boolean, hours: number) => {
    try {
      setError(null);
      ...
      const replace = (page: AdminPage<AdminAutoUpdateRule> | undefined) => page
  ...
  const enableDirectory = async (syncKey: string) => {
    try {
      ...
      qc.setQueryData<AdminPage<AdminAutoUpdateRule>>(DIRECTORY_QUERY_KEY, (page) => {
        if (!page) return { items: [created], total: 1, limit: 100, offset: 0 };
        const exists = page.items.some((item) => item.sync_key === syncKey);
        return {
          ...page,
          total: exists ? page.total : page.total + 1,
          items: exists
            ? page.items.map((item) => item.sync_key === syncKey ? created : item)
            : [...page.items, created],
        };
      });
      await refresh();
  ...
  const directoryByKey = new Map((directories.data?.items ?? []).map((rule) => [rule.sync_key, rule]));

  return <div className="space-y-7">
    {error && <p role="alert" className="text-sm text-danger">{error}</p>}
    <section>
      <h2 className="mb-3 text-base font-semibold text-ink">资产目录自动更新</h2>
      {directories.isError && <p role="alert" className="mb-2 text-sm text-danger">目录自动更新配置加载失败：{queryErrorMessage(directories.error)}</p>}
      {directoryUnits.isError && <p role="alert" className="mb-2 text-sm text-danger">目录清单加载失败：{queryErrorMessage(directoryUnits.error)}</p>}
      {directoryUnits.isLoading && <p className="mb-2 text-sm text-ink-muted">加载目录清单…</p>}
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1040px] text-sm">
          <thead><tr><th>目录</th><th>状态</th><th>周期</th><th>下次执行</th><th>最近成功</th><th>最近失败</th><th>任务</th><th>更新时间</th><th>操作</th></tr></thead>
          <tbody>{(directoryUnits.data ?? []).map((unit) => {
			...
			return <tr key={unit.sync_key} data-testid={`directory-rule-${unit.sync_key}`} className="border-t border-line">
			  <td>{unit.label}<div className="text-xs text-ink-muted">{unit.sync_key}</div></td>
              {rule
...
export default function AutoUpdatesPage() {
  return (
    <Suspense fallback={<p className="text-sm text-ink-muted">加载自动更新配置…</p>}>
      <AutoUpdatesContent />
    </Suspense>
  );
}
