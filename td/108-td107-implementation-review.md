...
后续版本补充：`research_optimizer_v4` 已将优化快照中的全部启用资产（含候选权重为 0 的资产）保留在统一有效估值日历中，所有候选使用同一市场样本；因此本文早期验收中的候选比较不应再理解为可使用各自不同的有效日期。`research_optimizer_v5` 在此共享日历上进一步计入研究交易成本。
