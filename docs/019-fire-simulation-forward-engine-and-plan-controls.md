...
当前引擎版本为 `3.5.0`，input snapshot 写入 `aggregate_cash_liquidity=true`：
...
- 路径只记录账本可证明的失败状态：`insufficient_funds`、`wealth_depleted`、`terminal_floor_not_met`、`other`，不再根据失败月份或累计通胀推断因果；3.4.0 的失败月份保留实际已支付支出、税、交易费和未满足支出，之后财富序列补零。
- 3.5.0 明确基金历史净值和 profile prior 已内含管理费、托管费等持续费用；资料库 `expense_ratio` 不进入快照、哈希或路径计算，买卖成本仍由 `transaction_cost_rate` 统一覆盖。
...
- 稳定收入先抵扣当月生活支出；只有净支出缺口需要从组合提取并产生提取税。超过支出的稳定收入进入现金池并参与当月收益。交易成本不属于生活支出，不能由稳定收入抵扣。

两个字段属于 config hash 和冻结 snapshot。修改任一字段会使旧 run stale；3.2.0
及更早 snapshot 缺少字段时按零收入回放，保持历史结果不变。

3.4.0 同时将随机通胀 AR(1) 初值设为长期均值 `mu`，并按失败月末计算
`failure_age_quantiles`（年龄精确到月）。3.3.0 及更早快照继续使用旧退休结算、
`AnnualPi=0` 初值、失败月截断序列和旧失败年龄语义，保证历史回放不漂移。
