...
4. 删除历史 migration 中只服务 `instrument_fetch/jobs` 的 DML 和索引逻辑；任务重构新增/修改语句只允许 DDL。
...
| 文件 | 固定修改 |
| --- | --- |
| `migrations/0001_init.sql` | 用目标 worker_tasks/versions/attempts 定义替换 jobs；simulation/analysis 使用 task_id |
| `migrations/0002_job_idempotency.sql` | 改为 `worker_task_idempotency_keys` DDL |
| `migrations/0004_instrument_fetch_status.sql` | 删除 jobs payload ALTER，保留编号为 DDL no-op |
| `migrations/0005_resolution_tickets_job_idempotency.sql` | 删除全部 jobs UPDATE/索引逻辑，只保留仍需要的 DDL 或改为 no-op |
| `migrations/0020_worker_tasks_market_assets.sql` | 删除重复 worker_tasks 创建，保留市场资产结构 |
| `migrations/0022_post_process_records.sql` | 改为 finalize records DDL |
| `migrations/0025_research.sql` | backtest run 使用 task_id，删除 run status |
| `migrations/0026_research_optimization.sql` | optimization run 使用 task_id，删除重复 status/error |

测试中 migration 数量仍保持当前编号总数；no-op 文件保留编号，禁止复用编号表达另一项业务变更。
...
只有同时满足以下条件才算 td/121 完成：

- 数据库只有 `worker_tasks` 一套可领取任务生命周期；
- 当前 9 类 task 全部在 TaskDefinition registry 注册并通过 E2E；
- sidecar 无主数据库访问能力；
...
- clean DB、故障注入、Go/Web/Python 全量测试全部通过；
- 实施 Review 记录每类任务的 task id、状态链、result key 和业务结果验证证据。
...
| task type | 状态链/result key | 业务结果证据 |
| --- | --- | --- |
| `simulation` | `pending -> running -> complete`；`simulation_run:{run_id}` | `TestSimulationJobFlow` 验证任务、run、路径和公共 task API |
| `stress` | `pending -> running -> complete`；`analysis_result:{task_id}` | `TestStressAndSensitivityJobFlow` 验证冻结输入和 analysis result |
| `sensitivity` | 同上 | `TestStressAndSensitivityJobFlow` |
| `research_backtest` | `pending -> running -> complete`；`research_backtest_run:{run_id}` | `TestResearchAPIFullBacktestFlow` 验证 points/months/summary |
| `research_optimization_backtest` | `pending -> running -> complete`；`research_optimization_run:{run_id}` | research optimization service/API 全量测试验证候选结果与 task join 状态 |
| `market_data_auto_update_scan` | `pending -> running -> complete`；`resource:{sha256}` | clean DB 现场任务 `task_703fa5ef-b8d7-4085-8d2a-2f8b36cb5bb2` 完成；summary resource 存在且幂等键绑定时间槽 |
| `asset_directory_sync` | `pending -> running -> pre_complete -> complete` | `TestTaskFinalizerDirectoryProtocolAndBusinessCommit` 验证目录、sync state、data version、finalize record |
| `asset_history_sync` | 同上 | `TestTaskFinalizerHistoryProtocolAndDuplicateResult` 验证 points、detail projection、data version、重复 result 不重复落库 |
| `fx_rate_sync` | 同上 | `TestTaskFinalizerFXProtocolAndBusinessCommit` 验证 USDCNY/HKDCNY points、双 data version、finalize record |

上述测试中的业务任务 ID 均由生产 producer 生成 `task_` 前缀 UUID；测试同时以 `task_id` 关联业务表并读取 `worker_tasks` 终态，不使用业务表独立状态字段。

### 18.3 故障与并发证据

- `TestConcurrentClaimHasSingleWinnerAndIdempotentRetry`：20 个并发 claim 仅一个成功。
- `TestExpiredLeaseRejectsHeartbeatUploadAndResultBeforeMaintenance`：lease 到期后，即使 maintenance 尚未扫描，旧 worker 也不能续租、上传或提交结果。
- `TestGracefulReleaseRequeuesAndClearsOwnership`：正常退出立即 requeue，并清空 owner/token/lease。
- `TestStartupAndExpiredLeaseRecovery`：Go 任务启动即恢复，未过期 sidecar 任务不被误杀。
- `TestDirectCompletionRollsBackWithBusinessTransaction`：业务写回滚时 task 不会错误 complete。
- `TestFinalizerReservationIsExclusive`、`TestFinalizerLimitFailsWithoutReturningEmptyReservation`：finalizer 单实例预约，达到次数/硬超时时直接 fail 且不产生空任务。
- sidecar `test_worker_protocol.py`：覆盖 claim conflict、lease lost 丢弃结果、result 传输失败保持心跳重试、accepted 停止心跳、shutdown release。
- `useTaskStatus.test.tsx`：覆盖 `pending/running/pre_complete/complete/failed/canceled` 六种状态。

### 18.4 Clean DB 现场结果

2026-07-12 已备份并重建 `.dev-data/fireman.db` 与 `.dev-data/fireman_resource.db`：

```text
schema_migrations = 30
jobs table count = 0
worker_tasks table count = 1
...
```text
task_id      = task_703fa5ef-b8d7-4085-8d2a-2f8b36cb5bb2
worker_type  = go_worker
type         = market_data_auto_update_scan
status       = complete
attempt      = 1/2
result_key   = resource:5f74d28d887875f261d0c088bd6328a6a266a1fcc51a3373f745c17d78b439d0
idempotency  = system/auto_update/market_data_auto_update_scan/1783868400000
```

resource DB 中存在对应 `application/json`、schema version 1 的 scan summary；attempt 审计结果为 `complete/success`。
...
cd web && npm run test:ci                                   PASS (81 files, 606 tests)
cd web && npm run build                                     PASS
cd sidecars/market-provider && uv run pytest -q             PASS
git diff --check                                            PASS
旧体系生产引用扫描                                           PASS (0 matches)
```

旧体系扫描范围包含 `jobs` SQL、`JobRepo`、`/api/v1/jobs`、`/admin/jobs`、`TaskDB`、sidecar `sqlite3`、`FIREMAN_DB_PATH`、`useJobStatus` 和 `useWorkerTaskPolling`。
