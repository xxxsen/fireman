...
| TickFlow 优先行情源 | 可选（默认关闭）：已解析 A 股与场内 `etf/index_etf` 在未复权口径下优先 `tickflow.klines:1d`，未命中自动回退 AKShare；LOF/公募基金/resolve 不接入 |
...
TickFlow 配置与 fallback 规则详见 `sidecars/market-provider/README.md`（td/074）。
