...
1. **规模状态条**（`|scale_gap| > 1 元`）：规模超出/缺口文案 + 同步基准 + 去补录持仓 + 暂不处理。
2. **结构偏差汇总表**：现状占比、结构还差、建议均基于 `structural_*`。
3. **折叠区「按计划规模对齐（高级）」**：只读展示 `plan_gap_*` / `plan_scale_action`。
4. 空状态：无持仓 / 结构与规模均一致 / 结构无建议但规模有偏。
...
- 已移除「目标金额 / 结构还差」列；页头「更新账户资产」链到变更向导
