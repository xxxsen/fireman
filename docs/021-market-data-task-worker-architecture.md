...
目录同步以**目录同步单元（`sync_key`）**为真实任务粒度，`cn_all/hk_all/us_all` 只是 UI 聚合视图（详见 `td/090`）。单元由 Go 层静态 registry 定义，前端不能提交自定义 `markets/instrument_types`：

| sync_key | scope | instrument_types | 上游 |
| --- | --- | --- | --- |
| `cn_exchange_stock` | `cn_all` | `cn_exchange_stock` | `ak.stock_sh_a_spot_em` / `ak.stock_sz_a_spot_em` / `ak.stock_bj_a_spot_em` |
