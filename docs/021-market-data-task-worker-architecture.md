...
| --- | --- | --- |
| `cn_all` | `cn_exchange_stock`、`cn_exchange_fund`、`cn_mutual_fund` | `ak.stock_sh_a_spot_em` / `ak.stock_sz_a_spot_em` / `ak.stock_bj_a_spot_em`；`em.cn_etf_list` / `em.cn_lof_list`（保留 Eastmoney `f13` 市场标识）；`ak.fund_name_em` |
| `hk_all` | `hk_stock`、`hk_etf` | HKEX List of Securities（身份字段），`em.hk_equity_list` / `em.hk_fund_list`（中文展示名补充） |
...
- CN 股票交易所来自分交易所 spot board 本身（SH/SZ/BJ）。
...
- **资产目录 `/assets`**：固定展示三个 scope 同步行 + FX 同步行（状态徽章、最近成功时间、失败错误码、轮询错误提示；全部成功时默认折叠），代码/名称/市场/类型筛选、「含已退市」过滤、分页跳转、7 天未同步提醒；HK/US ETF 与股票同列展示，可直接进入详情。
