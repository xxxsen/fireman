...
- 配置模板页（`app/scenarios/page.tsx`）移除地区组内权重编辑，仅编辑大类目标权重（保存时仍提交默认 region targets 以兼容现有接口）。
- 计划设置“目标配置”（`PlanTargetsContent`）：配置模板只读预览大类权重；新增“本计划国内/国外配比”可编辑区，读取/编辑 `allocation.region_targets`。
  - 切换场景仅预览新模板大类权重，地区配比保持计划现值。
  - 保存：场景变更调用 `applyScenario`（用返回的 `config_version`）；地区变更调用 `updateAllocation`。

## 4. 计划设置目标配置 UI
- 移除独立的“前往配置模板”按钮，改为内联文案提示可前往 `/scenarios` 修改模板结构。
