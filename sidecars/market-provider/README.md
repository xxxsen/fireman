...
- 市场数据统一通过 Go 内部任务 API 流转：worker 拉取候选、claim、上报心跳、上传 task-bound gzip 结果并上报 result；Go 将成功结果置为 `pre_complete`，随后由 Go finalizer 完成业务落库。
...
worker 生命周期：按 `sidecar_worker/pending` 拉候选 → 使用随机 claim token 领取 → 每 10 秒通过 Go API 上报心跳 → task-bound resource 上传 → result 上报。Go 接受成功结果后进入 `pre_complete`，sidecar 停止心跳；lease 恢复、finalizer 重试与终态均由 Go 负责。sidecar 不挂载、读取或写入主 SQLite。
...
| 环境变量 | 默认 | 说明 |
| --- | --- | --- |
| `FIREMAN_WORKER_ENABLED` | `true` | 是否启动 API 驱动的 worker 循环 |
| `FIREMAN_INTERNAL_API_URL` | `http://backend:8081` | Go 内部任务 API 地址 |
